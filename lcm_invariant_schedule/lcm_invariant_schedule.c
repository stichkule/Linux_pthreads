/*
* @file lcm_invariant_schedule.c
*
* @brief Implements a LCM invariant schedule for two tasks
* @details Two tasks fib10 and fib20 with run-times of 10 ms and 20 ms are created. They have periodic request rates of 20 ms and 50 ms, respectively.
* A LCM invariant schedule is implemented over the LCM of the two periods (100 ms), and using static priorities, the feasibility of the schedule is determined.
* 
* @date 06 February, 2018
*
* @author Nisheeth Bhat
...
#include <math.h>
#include <sys/param.h>
//#define FIB_TIMER_CONFIG /* configure number of iterations for FIB_TEST */
...
//start = readTOD();
//mdelay(10);
FIB_TEST(seqIterations,1310000);
stop = readTOD();
pthread_getschedparam(testThread10,&policy ,&param);
#ifdef SYSLOG
...
void *Thread20(void *threadid)
{
	...
	while (!abortTest_20)
	{
		...
		//start = readTOD();
		FIB_TEST(seqIterations,2670000);
		stop = readTOD();
